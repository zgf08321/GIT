...
{   
    ...
    while (GPIO_ReadInputDataBit(GPIOA,GPIO_Pin_15)&&retry<200)
	{
		retry++;
		delay_us(1);
	};	 
    ...
    while (!GPIO_ReadInputDataBit(GPIOA,GPIO_Pin_15)&&retry<240)
	{
		retry++;
		delay_us(1);
	};
...
u8 DS18B20_Read_Bit(void) 			 // read one bit
{
	...
	DS18B20_IO_IN();//SET PA0 INPUT
	delay_us(12);
	if(GPIO_ReadInputDataBit(GPIOA,GPIO_Pin_15))data=1;
    else data=0;	 
    delay_us(50);           
    return data;
}
